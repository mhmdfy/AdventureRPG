...
/*
 * This is a copy of ThirdPersonCamera.
 * I copied it in order to change it if needed while having a
 * backup unchanged copy
 */
...
private var controller : TPController;
...
function Awake ()
{
	...
	if(!cameraTransform) {
		Debug.Log("Please assign a camera to the TPCamera script.");
		enabled = false;	
	}
	...
	if (_target)
	{
		controller = _target.GetComponent(TPController);
	}
	...
	if (controller)
	{
		...
		Debug.Log("Please assign a target to the camera that has a TPController script attached.");
